...
class LogParser:
    ...
    def parse_log_file(self, file_path: Path) -> list[dict]:
        ...
        try:
            ...
            st.error(f"Error parsing {file_path.name}: {e!s}")

        return messages

    def parse_all_logs(self, selected_files: list[Path] | None = None) -> pd.DataFrame:
    ...
    if search_term:
        if search_type == "Contains":
            ...
            try:
                ...
                st.error(f"Invalid regex pattern: {e!s}")
        ...
        if st.sidebar.button("Download Filtered Data"):
            if export_format == "CSV":
                ...
                st.sidebar.download_button(
                    label="📥 Download CSV",
                    data=csv,
                    file_name=f"ai_conversations_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.csv",
                    mime="text/csv",
                )
                ...
                st.sidebar.download_button(
                    label="📥 Download JSON",
                    data=json_data,
                    file_name=f"ai_conversations_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.json",
                ...
                st.sidebar.download_button(
                    label="📥 Download TXT",
                    data=txt_data,
                    file_name=f"ai_conversations_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.txt",
                    mime="text/plain",
                )

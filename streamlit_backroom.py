...
class OllamaClient:
    ...
    async def test_connection(self) -> tuple[bool, list[str]]:
        ...
        try:
            async with (
                ...
                session.get(f"{self.base_url}/api/tags", timeout=aiohttp.ClientTimeout(total=10)) as response,
            ):
    ...
    async def generate_stream(
        self, model: str, prompt: str, system: str | None = None, think: bool = True, timeout: int = 300
        ...
        try:
            ...
            try:
                async with session.post(
                    f"{self.base_url}/api/generate", json=payload, timeout=aiohttp.ClientTimeout(total=timeout)
                ) as response:
                    if response.status == 200:
                        try:
                        ...
                        if "does not support thinking" in error_text:
                            ...
                            yield {
                                "type": "info",
                                "content": f"Model {model} doesn't support thinking - switching to standard mode for future requests",
                            }
                ...
                with st.expander(f"{avatar} {persona.name} ({persona.model}){role_display}", expanded=False):
                    ...
                    with col2:
                        if persona.role and persona.role in self.role_templates:
                            st.text_area(
                                "Role Description",
                                value=self.role_templates[persona.role],
                    ...
                    with col3:
                        ...
                        if not st.session_state[delete_key]:
                            if st.button(
                                "🗑️ Delete", key=f"delete_{persona.id}", type="secondary", help="Delete this persona"
                            ):
        ...
        with st.form("new_persona_form"):
            ...
            with role_col1:
                ...
                selected_role = st.selectbox(
                    "Predefined Role", role_options, format_func=lambda x: x if x else "No specific role"
                )
    ...
    def settings_ui(self):
        ...
        with st.form("settings_form"):
            ...
            max_history = st.number_input(
                "Max History Messages", min_value=10, max_value=200, value=st.session_state.settings["max_history"]
            )
            ...
            auto_advance = st.checkbox("Auto-advance conversation", value=st.session_state.settings["auto_advance"])
        ...
        if st.session_state.non_thinking_models:
            ...
            if st.button(
                "🔄 Reset Model Compatibility Cache", help="Clear the cache of models that don't support thinking"
            ):
    ...
    def conversation_ui(self):
        ...
        if prompt := st.chat_input("Add a message to the conversation (optional)"):
            ...
            st.session_state.messages.append(
                {"role": "user", "content": prompt, "timestamp": timestamp, "persona_name": "User", "model": "Human"}
            )
        ...
        if st.session_state.is_running and st.session_state.settings["auto_advance"]:
            ...
            with st.status(
                f"🔄 Auto-running conversation... (Turn {st.session_state.auto_run_count})", expanded=True
            ) as status:
                ...
                delay = random.uniform(
                    st.session_state.settings["response_delay_min"], st.session_state.settings["response_delay_max"]
                )
...
Your response should be conversational and engaging."""
    ...
    def run(self):
        ...
        st.set_page_config(page_title="AI Backroom", page_icon="🤖", layout="wide", initial_sidebar_state="expanded")
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["💬 Conversation", "🤖 Personas", "⚙️ Settings", "📁 Export & Logs"])

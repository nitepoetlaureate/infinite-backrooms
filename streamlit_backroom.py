...
from ui_design_tokens import mention_highlight_style, persona_badge_style
    ...
    async def generate_stream(
        ...
        try:
            ...
            try:
                ...
                yield {"type": "error", "content": f"Error: {e!s}"}
        except asyncio.CancelledError:
            ...
            for i, persona in enumerate(st.session_state.personas):
                ...
                with st.expander(
                    ...
                    with col1:
                        ...
                        st.color_picker(
                            "Color", value=persona.color, key=f"color_{persona.id}", disabled=True
                        )
                    ...
                    with col3:
                        ...
                        if not st.session_state[
                            delete_key
                        ]:  # Only show if not in delete confirmation mode
        ...
        with st.form("new_persona_form"):
            ...
            with role_col2:
                if role and role in self.role_templates:
                    st.text_area(
                        "Role Description",
                        value=self.role_templates[role],
                        disabled=True,
                        height=100,
                    )
            ...
            if message["role"] == "user":
                ...
                with st.chat_message("assistant", avatar=avatar):
                    ...
                    if "@" in content and any(
                        f"@{p.name}" in message["content"] for p in enabled_personas
                    ):
            ...
            try:
                ...
                async def process_stream():
                    ...
                    try:
                        ...
                        st.error(f"Stream processing error: {e!s}")
                        return
                ...
                st.error(f"Processing error: {e!s}")
            finally:
    ...
    def export_ui(self):
        ...
        if st.session_state.messages:
            ...
            st.download_button(
                ...
                data=json_str,
                file_name=f"streamlit_backroom_session_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}.json",
        ...
        if log_file.exists():
            ...
            st.download_button(
                ...
                data=log_content,
                file_name=f"streamlit_backroom_log_{datetime.now(UTC).strftime('%Y-%m-%d')}.txt",
                mime="text/plain",
            )
        else:
